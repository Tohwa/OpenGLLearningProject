...
// Initializes the engine, sets up the viewport, input, and OpenGL settings
int SEngine::Initialize(void)
{
    ...
    // Disable cursor and capture it within the window
    glfwSetInputMode(m_Viewport.GetWindow(), GLFW_CURSOR, GLFW_CURSOR_DISABLED);

    // Initialize input and time systems
    Input::Initialize(m_Viewport.GetWindow());
    Time::Initialize();

    // Set depth function and enable multisampling
    glDepthFunc(GL_LESS);
...
// Main loop for running the engine
int SEngine::Run(void)
{
    // Texture holder for managing textures
    ...
    // Frame buffer for blur effects
    BlurFrameBuffer blurFrameBuffer{ 1280, 720 };
    blurFrameBuffer.Initialize();

    // Quad view for rendering full-screen quads
    QuadView quadView{};
    ...
    // Initialize game objects (planets and sun)
...
#pragma endregion
    ...
    // Create quad buffer for post-processing
    ...
    while (!glfwWindowShouldClose(m_Viewport.GetWindow())) {
        ...
        // Update viewport, camera, and game objects
        m_Viewport.Update();
        camera.Update();
        sun.Update();
        mercury.Update();
        ...
        // Clear screen and draw final quad with blend shader
        ...
        // Late draw updates for the viewport
        m_Viewport.LateDraw();
    ...
    // Finalize frame buffer
    framebuffer.Finalize();

    return 0;
}

// Finalizes the engine, cleaning up resources
